...
class EditEbookAdmin extends StatefulWidget {
  ...
  @override
  State<EditEbookAdmin> createState() => _EditEbookAdminState();
}

class _EditEbookAdminState extends State<EditEbookAdmin> {
  ...
  void initState() {
    ...
    _bookMonthController.text = widget.data.get('month');
    _bookYearController.text = widget.data.get('year');
                ...
                onPressed: () async {
                  ...
                  if (_formState.currentState!.validate()) {
                    ...
                    await uploadFileToFireStore(
                        fileUrl: widget.data.get('fileUrl'), name: '$id.jpg');
                  } else {
                    //
                    task = putFileToFireStorage(
                      file: selectedFile!,
  ...
  uploadFileToFireStore({
    ...
    var id = widget.data.id;
    //
    FirebaseFirestore.instance.collection('ebook').doc(id).update({
      'id': id,
      'title': _bookTitleController.text.trim(),
      'month': _bookMonthController.text.trim(),
      'year': _bookYearController.text.trim(),
